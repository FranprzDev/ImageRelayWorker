...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("/api/config", func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]any{"saved": true, "workerRestarted": true})
	})
...
const $=id=>document.getElementById(id); async function health(){let h=await fetch('/api/health').then(r=>r.json());$('status').textContent=h.ok?'Web activa · Worker '+(h.workerRunning?'ejecutándose':'detenido'): 'Error'} async function load(){await health();let c=await fetch('/api/config').then(r=>r.json());$('api').value=c.APIBaseURL||'';$('id').value=c.WorkerID||''} document.querySelector('form').onsubmit=async e=>{e.preventDefault();let c={APIBaseURL:$('api').value,WorkerToken:$('token').value,WorkerID:$('id').value};let r=await fetch('/api/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(c)});alert(r.ok?'Guardado. Worker reiniciado con la nueva configuración.':await r.text());if(r.ok){$('token').value='';await health()}};load();setInterval(health,5000);
</script></body></html>`
